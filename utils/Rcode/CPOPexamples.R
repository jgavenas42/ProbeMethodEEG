...
#
###
##To install l1tf code from github 
library(devtools)
install_github("hadley/l1tf")##obtained from https://github.com/hadley/l1tf


library("not") ##library for narrowest-over-threshold
library("l1tf") ##library for trend-filtering
library(cpop) ##library for cpop
library(splines)
...
n=length(z)
out=CROPS.CPOP(z,sig.hat^2,min_pen=5,max_pen=2*log(n),PRINT=T) 
